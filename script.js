class Movie {
  constructor() {
    ...
    this.actors.hatConfettiLeft = this.actors.hatConfettiGroup.querySelectorAll(
      "#hat-confetti circle.left"
    );
    this.actors.hatConfettiRight =
      this.actors.hatConfettiGroup.querySelectorAll(
        "#hat-confetti circle.right"
      );
    this.actors.balloonConfetti = document.querySelector("#balloon-confetti");
    this.actors.balloonConfettiLeft =
      this.actors.balloonConfetti.querySelectorAll(
        "#balloon-confetti circle.left"
      );
    this.actors.balloonConfettiRight =
      this.actors.balloonConfetti.querySelectorAll(
        "#balloon-confetti circle.right"
      );
    this.actors.gopher = document.querySelector("#gopher");
    this.actors.gopherRightEye =
      this.actors.gopher.querySelector("#go-right-eye");
    this.actors.gopherLeftEye =
      this.actors.gopher.querySelector("#go-left-eye");
    this.actors.gopherLeftHand =
      this.actors.gopher.querySelector("#go-left-hand");
    this.actors.gopherEyeBlinks =
      this.actors.gopher.querySelectorAll(".eye-blink");
  ...
  _startEyes() {
    // start eye blinks...
    this.tweens.gopherEyeBlinks = TweenMax.to(
      this.actors.gopherEyeBlinks,
      0.6,
    ...
    stage.mousemove((e) => {
      var radius = 28,
        ...
        my = e.screenY,
        gopherX =
          this.actors.gopher.getBBox().x + this.actors.gopher.getCTM().e,
        gopherY =
          this.actors.gopher.getBBox().y +
          this.actors.gopherRightEye.getBBox().height +
          this.actors.gopher.getCTM().f,
        flip = mx < gopherX ? -1 : 1,
        deg = Math.atan((my - gopherY) / (mx - gopherX)),
        dy = radius * Math.sin(deg) * flip,
        dx = radius * Math.cos(deg) * flip + xoff;

      TweenMax.set(this.actors.gopherPupils, {
        x: dx,
        y: dy,
  ...
  _redrawString() {
    ...
    var start = { x: hand.getBBox().x - 212, y: hand.getBBox().y + 130 },
      ...
      balloon.getBBox().x +
      balloon.getBBox().width / 2 +
      balloon._gsTransform.x;
    end.y =
      balloon.getBBox().y + balloon.getBBox().height + balloon._gsTransform.y;

    var control = {
      x: start.x + Math.abs(end.x - start.x) * 1.2,
      y: start.y - Math.abs(end.y - start.y) * 0.4,
    };
  ...
  _balloonDrag(me) {
    ...
    var tween = Draggable.create(me.actors.balloon, {
      ...
      onDrag: function (e) {
        ...
        var confetti = [
          ...me.actors.balloonConfettiLeft,
          ...me.actors.balloonConfettiRight,
        ];

        var dx =
            me.actors.balloon.getCTM().e +
            me.actors.balloon.getBBox().width * 0.1,
          dy =
            me.actors.balloon.getCTM().f +
            me.actors.balloon.getBBox().height * 0.1;

        TweenMax.staggerTo(
          confetti,
          1,
          { x: dx, y: dy, ease: Power4.easeOut },
          0.01
        );
      },
  ...
  setStage() {
    ...
    clearTl
      ...
      .set([actors.balloonConfettiLeft, actors.balloonConfettiRight], {
        autoAlpha: 0,
      })
  ...
  enterPartyHat() {
    ...
    hatTl
      .fromTo(
        ...
        0.6,
        { scale: 0.2 },
        { scale: 1, autoAlpha: 1, ease: Elastic.easeInOut }
      )
      .add("hat")
      .staggerFromTo(
        actors.hatConfettiLeft,
        1,
        { x: "+=50", scale: 0.05 },
        { x: 0, scale: 1, autoAlpha: 1, ease: Power4.easeOut },
        0.01,
        "hat-=0.4"
      )
      .staggerFromTo(
        actors.hatConfettiRight,
        1,
        { x: "-=50", scale: 0.05 },
        { x: 0, scale: 1, autoAlpha: 1, ease: Power4.easeOut },
        0.01,
  ...
  enterBalloon() {
    ...
    TweenMax.set(actors.balloonString, {
      strokeDasharray: slength,
      strokeDashoffset: slength,
    });

    balloonTl
      .to(actors.balloonString, 0.4, {
        strokeDashoffset: 0,
      ...
      .add("balloon")
      .staggerFromTo(
        actors.balloonConfettiLeft,
        1.4,
        { x: "+=50", scale: 0.05 },
        { x: 0, scale: 1, autoAlpha: 0.9, ease: Power4.easeOut },
        0.01,
        "balloon-=0.5"
      )
      .staggerFromTo(
        actors.balloonConfettiRight,
        1.4,
        { x: "-=50", scale: 0.05 },
        { x: 0, scale: 1, autoAlpha: 0.9, ease: Power4.easeOut },
        0.01,
  ...
  enterGreeting() {
    ...
    greetTl.to(actors.greeting, 1, {
      autoAlpha: 1,
      onComplete: this._balloonDrag,
